...
3. `resolveConsent` / `sendStats` (`src/stats.ts`) implement opt-in anonymous usage stats posted to `https://stats.adamczyk.ovh`, mirroring `skillslint/src/helpers/stats.ts`. The body is one constant — `{ tool: 'structurelint', options: [] }` — for CLI and API alike: a run count is the whole question, so nothing about how the run was invoked goes out and there is nothing to sanitize. `resolveConsent` is tri-state (`true`/`false`/`undefined` = never asked), reads and writes `stats` in the structure config (spliced into the user's source via regex, not re-serialized), and only prompts — via `readline/promises` — with a TTY on stdin and stdout, outside CI, and without `--json`. The CLI resolves consent and awaits the send itself, because `process.exit` would kill the fire-and-forget request a library call can afford; `lint()` fires `void sendStats()` when its **required** `stats` argument is `true` and never prompts — `sendStats` stays package-internal so consent is checked in one place, which is why `lint()` also takes an optional `config` (QoQ holds the structure inline in `qoq.config.*` and would otherwise have to call `validate()` and count the run itself).
4. `templateToRegex` / `globToRegex` (`src/helpers/matcher.ts`) compile rule-name templates (`{PascalCase}`, `(ts|tsx)`, `*`) and ignore globs (`*`, `**`, `?`) into anchored regexes.
